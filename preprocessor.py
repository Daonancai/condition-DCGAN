...
class Dataset:
    ...
    def __init__(self, imagePath, extensions):
        ...
        self.labels, self.inputs = zip(*self.data)#to get all labels and image paths #self.labels like [cat,cat,...dog,...]

        category = np.unique(self.labels)#delete the labels that are the same
        self.num_labels = len(category)
        self.category2label = dict(zip(category, range(len(category))))
        self.label2category = {l: k for k, l in self.category2label.items()}# like {0: 'cat', 1: 'dog'}

        # Convert the labels to numbers
        self.labels = [self.category2label[l] for l in self.labels]#self.labels like [0,0,0,...,1,...]
    ...
    def nextRecord(self):
        ...
        input = self.preprocess(cv2.imread(self.inputs[self.next_record],-1))
        self.next_record += 1
        return label, input#input.flatten()
